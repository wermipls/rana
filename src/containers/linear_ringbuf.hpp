...
class LinearRingBuf {
    ...
    constexpr static size_t push_offset = sz & mask;
...
public:
    ...
    void push(const T &v) { auto i = (offset + push_offset) & mask; buf[i] = v; buf[i+capacity/2] = v; offset = (offset+1) & mask; }
    void push(T &&v)      { auto i = (offset + push_offset) & mask; buf[i] = v; buf[i+capacity/2] = v; offset = (offset+1) & mask; }
